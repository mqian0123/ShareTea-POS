...
function CartModal ({onClose, orderList, incrementQuantity, decrementQuantity, deleteItem, calculateTotal, totalPoints, customerID}) {
    ...
    const [deductedUserPoints, setDeduction] = useState(0);
    ...
    useEffect(() => {
        // userPoints calculation is: every $1 spent = 100 points, 1000 points ~ $1
        if (applyDiscount) {
            let total = (subtotal + tax - (userPoints / 1000)).toFixed(2);
            if (total >= 0) { // not enough userPoints to overflow
                setFinalPrice(total);
                setDeduction(userPoints);
            }
            else { // reduce userPoints by finalPrice.
                setFinalPrice(0);
                setDeduction(Math.ceil((subtotal + tax)*1000))
            }
        } 
        else {
    ...
    const placeOrder = async () => {
        const data = {
            ...
            points: Math.floor(calculateTotal() * 100),
            cost: finalPrice,
            customerID: customerID,
            employeeID: 21, // self-service kiosk id = 21 in the database
            orderList: orderList
        }
        // console.log(data);
        axios.post(SERVER_API + 'cashier/addOrder', data).catch(error => {
            console.log(error);
        });
                    ...
                    <div className="relative p-4 w-full max-w-2xl max-h-full">
                        <div className="relative text-gray-900 bg-white border font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 ">
                                ...
                                applyDiscount && (
                                    <>
                                        <div className='flex justify-between py-2.5'>
                                            ...
                                            <p className='font-bold'>
                                                -{deductedUserPoints} Points (${(deductedUserPoints / 1000).toFixed(2)})
                                            </p>
                                        </div>
                                        <hr>
                                        </hr>
                            ...
                            <div class="text-white bg-[#050708] hover:bg-[#050708]/80 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:hover:bg-[#050708]/40 dark:focus:ring-gray-600 me-2 mb-2">
                                <input onChange = {() => setPaymentMethod('applePay')} id="applePay" type="radio" value="Card" name="default-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 mr-5" />
                            ...
                            <div class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-800 dark:bg-white dark:border-gray-700 dark:text-gray-900 dark:hover:bg-gray-200 me-2 mb-2">
                                <input onChange={() => setPaymentMethod('card')}  id="cash" type="radio" value="Cash" name="default-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 mr-5" />
                                ...
                                <label for="cash" class="ms-2 text-sm font-medium text-gray-900 ">Pay with Card</label>
                            </div>
                                {/* Footer with add to order and cancel buttons */}
                                <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                                
                                {/* TODO: placeOrder method onClick for this button: */}
                                <button onClick={placeOrder} data-modal-hide="static-modal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Proceed to Checkout</button>                                
